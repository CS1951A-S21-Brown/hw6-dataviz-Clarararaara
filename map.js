...
]).then (function ([tsvData, topoJSONdata, gameData]){
    gameData.forEach(d=> {d.Rank = parseInt(d.Rank);
...
//          groups.exit().remove();
         ...
         group.append("text")
            ...
            .text('Top Genre in:')
            .style('fill', '#635F5D');
//Globally is '+ globaltopdata[0].Genre);
        };
...
// set up mouseover and mouseout func
       ...
       // Set opacity back to 0 to hide
    	...
    	.data(countries)
  		...
  		;
       // Add a title
       svgG.append('g').attr('transform', `translate(${margin.left}, ${margin.top})`)
       .append("text")
       .attr('class', 'title')
        .attr("transform", `translate(${(innerwidth_2/2)},-15)`)
        .attr("text-anchor", "middle")
        .text("Most Popular  " + platdisplay(select_plat) +' Game ' + "Genre in 4 Sales Regions " + yeardisplay(start, end));
